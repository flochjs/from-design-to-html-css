...
const useState = (defaultState = null) => {
  let state = defaultState;

  return [() => state, (value) => void (state = value)];
};
...
const activeItem = (item) => {
  document.body.style.background = item.getAttribute('data-bg');
...
const handleArrowToggling = (
  ...
  if (wasFirst) enableArrow(leftArrow);
  else if (wasLast) enableArrow(rightArrow);

  if (willBeFirst) disableArrow(leftArrow);
  else if (willBeLast) disableArrow(rightArrow);
};
...
const toggle = (
  ...
  if (!isInfinite) handleArrowToggling(...indexInfos, leftArrow, rightArrow);
...
const goTo = (
  ...
  toggle(
    ...
    activeIndex,
    newActiveIndex,
    [wasFirst, wasLast, willBeFirst, willBeLast],
    isInfinite,
    items,
    indicators,
    leftArrow,
  ...
  else moveItemsRight(newActiveIndex, activeIndex, items);
};

const handleCycling = (getActive, items, makeCycle) => () => {
  const activeIndex = getActive();
  if (!items[activeIndex].matches(':hover')) makeCycle();
};

const handleCarouselControls = (
  isInfinite,
  interval,
  resetInterval,
  makeCycle,
) => () => {
  if (isInfinite && interval) resetInterval();
  makeCycle();
};
...
for (const carousel of carousels) {
  ...
  const [leftArrow, rightArrow] = carousel.getElementsByClassName(
    CLASSNAME.arrow,
  );
  ...
  if (!isInfinite) disableArrow(leftArrow);
  ...
  const [_setInterval, resetInterval] = useInterval(
    handleCycling(getActive, items, goTo(...goToParams, getNext)),
    interval,
  );
  ...
  if (isInfinite && interval) _setInterval();
}
